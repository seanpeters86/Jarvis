  ...
  ranking: function(parsed, parsedReg, input, user, message) {
    ...
    request({
        ...
        if (!error && response.statusCode == 200 && (encounter != 0) && body.length > 0) {
            ...
            if (input.includes("-H")) {
              console.log(parsed[1] + " ranked " + `${prettyjson.render(rank[0].rank)}` + " out of " + `${prettyjson.render(rank[0].outOf)}` + " on " + bossname + " for all " + specparsed + " " + classparsed + "s in HPS");
              ...
              console.log(parsed[1] + " ranked " + `${prettyjson.render(rank[0].rank)}` + " out of " + `${prettyjson.render(rank[0].outOf)}` + " on " + bossname + " for all " + specparsed + " " + classparsed + "s in DPS");
